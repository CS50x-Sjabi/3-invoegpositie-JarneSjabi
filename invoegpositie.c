...
int searchInsert(int *nums, int numsSize, int target)
{
    ...
    // Not found in array: return the index where it should be (order in numbers)
    int previousValue = 0;
    for (int index = 0; index < numsSize; index++)
    {
        int currentIterValue = nums[index];
        if (target <= currentIterValue)
        {
            return index;
        }
        previousValue = currentIterValue;
    }
    return numsSize;
}
    ...
    for (int i = 0; i < numTests; i++)
    {
        ...
        if (result == test.expected)
        {
            printf("✅ Test %d geslaagd: target = %d, verwacht = %d, output = %d\n", i + 1,
                   test.target, test.expected, result);
            passed++;
        }
        else
        {
